...
```{r ggplot2-eg, echo = FALSE, fig.height=4, fig.width=10, message = FALSE}
...
* _Set_ some parameters: alpha of the points to avoid overplotting and see the smooth fit better
</aside>
...
```{r base-plus-smooth, echo = T, fig.align='center', fig.width=10, fig.height=4, message = FALSE, warning = FALSE}
ggplot(diamonds, aes(carat, price, colour = clarity)) +
  geom_smooth()
```
...
```{r base-facets-scales, echo = T, fig.align='center', fig.width=10, fig.height=4, message = FALSE}
ggplot(diamonds, aes(carat, price, colour = clarity)) +
  geom_point(alpha = .015) + geom_smooth() +
  scale_y_continuous(labels = scales::comma)
```
...
<div class="notes">
* Let's specify _how_ to map from clarity to colour, e.g. what colours to use:
* We can use some specific colours, e.g. to brand the plot with KPMG colours
* Bit of a hack for now - will come up with better way to get colours in future
...
* Add layers to control: geoms, facet specs, scales, visuals (labels, themes)
* Come to training to learn more
</div>

## Finished plot

```{r ggplot_eval, eval = T, echo = F, fig.height=5, fig.width=10, message = FALSE}
ggplot(diamonds, aes(carat, price, colour = clarity)) +
  geom_point(alpha = .015) + 
  geom_smooth() +
  scale_y_continuous(labels = scales::comma) +
...
1. Clone/download the session [materials](https://hdp-service.uk.kworld.kpmg.com/stash/projects/JL/repos/r-training/browse)
2. Re-create example plots
3. Create your own plots for every-day use
4. Profit
